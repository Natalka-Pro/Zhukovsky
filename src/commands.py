...
def loop(model, CONF, dataset, margin, train=True, kind="siam"):
    ...
    train_loader = DataLoader(train_dataset, batch_size=CONF.loader.batch_size, shuffle=True)
    test_loader = DataLoader(test_dataset, batch_size=CONF.loader.batch_size, shuffle=False)
    ...
    if kind == "siam":
        ...
        optimizer = torch.optim.Adam(model.parameters(), lr=CONF.classifier.learning_rate)

    if train:
        model = train_fun(model, train_loader, test_loader, loss, optimizer, 
                          n_epochs, CONF.device)
...
def pos_neg_dataset(CONF):
    ...
    pos_dataset = My_Dataset("pos", CONF.dataset.data_pos, 
    ...
    neg_dataset = My_Dataset("neg", CONF.dataset.data_neg, 
...
# if __name__ == "__main__":
    ...
    dataset = TripletDataset(pos_dataset, neg_dataset, 
                             required_len=1000, 
                             deterministic=True,
                             seed=CONF.seed)
    ...
    cl = nn.Sequential(nn.Linear(1000, 512),
                       nn.ReLU(),
                       nn.Linear(512, 2)).to(CONF.device)
